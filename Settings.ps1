...
if( $Env:USER1_AUTH_TOKEN -eq $env:UserName) {
        ...
        Set-CurrentUser -Save -Username $Env:USER1_AUTH_TOKEN -Password $Env:USER1_AUTH_TOKEN -WSURL wss://robot.shebeng.co -disable_instance_store true
...
if( $Env:USER2_AUTH_TOKEN -eq $env:UserName) {
        ...
        new-item $env:USERPROFILE\Documents\OpenRPA -itemtype directory
        while (!(Test-Path "C:\Program Files\OpenRPA\OpenRPA.exe")) { Start-Sleep 5 }
        Set-CurrentUser -Save -Username $Env:USER2_AUTH_TOKEN -Password $Env:USER2_AUTH_TOKEN -WSURL wss://robot.shebeng.co -disable_instance_store true
        Start-Process "OpenRPA.exe"
